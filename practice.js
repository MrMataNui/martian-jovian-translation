$(function(){
	...
	function jovTranslate() {
		...
		function puncTest (val, key, punc) {
			var test = false;
			$.each(key, function(funct_key, funct_val) {
				if (val == key + punc
					|| val == punc + key
					|| val == punc + key + punc )
					test = true;
		...
		function punctu (val, key1, dict, dict_key, punc) {
			$.each(key1, function(funct_key, funct_val) {
		...
		$.each(split_text, function (i, split_val) {
			if (split_val > -Infinity && split_val < Infinity) {
				$.each(numWords, function (num_key, num_val) {
		...
		$.each(placementArray, function (i, val) {
			...
			if (place)
			$(place).each(function () {
				$.each(solutionArray, function (sol_key, sol_val) {
				...
				$.each(solutionArray2, function (sol_key, sol_val) {
				...
				var tensePunc;
				$.each(solutionArray3[i][0], function (sol_key, sol_val) {
				...
				$.each(solutionArray2, function (sol_key, sol_val) {
					...
					if ( !solutionArray[sol_key] ) {
						...
						$.each(solutionArray[sol_key], function (key, val) {
							if ( val == sol_val[key] )
								solNum += 1;
						});
