...
function conlangTranslate(dict, letters, vowels) {
	...
	$.each(split_text, (i, split_val) => {
		...
		$.each(punctuationSelf, (punct_key, punct_val) => {
	...
	$.each(placementArray, (i, val) => {
		...
		$(place).each(() => {
			...
			var tensePunc;
			$.each(solutionArray[i + 1], (sol_key, sol_val) => {
	...
	$('#div').children('p.translation').text((_, txt) => {
		...
		if (dictType === 'Jove') {
			...
			$.each(txt, (txt_key, txt_val) => {
				...
				$.each(EndPunctuation, (punc_key, punc_val) => {
		...
		if ( nextPunc2 != [] ) {
			...
			$.each(nextPunc2, (nextPunc_key, nextPunc_val) => {
				if (nextPunc_key > 0) {
